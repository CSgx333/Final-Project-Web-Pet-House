...
function UserPetLostPost({ setActiveTab }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const lostData = allData.filter(item => item.Account_id === parseInt(UserId));

                // กรองจากชื่อ
                const lostName = lostData.filter(item =>
                    item.Pet_name.toLowerCase().includes(searchName.toLowerCase())
                );

                // กรองจากสายเพศ
                const lostGender = filterGender ? lostName.filter(item => item.Gender === filterGender) : lostName;
                setLostData(lostGender);
            } catch (error) {
    ...
    const handleDelete = (petLostId, PetId) => {
        Swal.fire({
            title: 'Are you sure ?',
            ...
            if (result.isConfirmed) {
                ...
                axios.delete('http://localhost:5000/pet_lost', { data: data })
                const updatedLostData = lostData.filter((item) => item.Pet_lost_id !== petLostId);
                setLostData(updatedLostData);
    ...
    const handleEditStatus = (PetLostId) => {
        Swal.fire({
            title: 'Are you sure ?',
            text: 'You won\'t be able to revert this !',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                axios.post('http://localhost:5000/update_status_petLost', { ...inputs,
                    PostStatus: 2,
                    PetLostId: PetLostId
                })
                const swal1 = Swal.fire({
        ...
        <div class="bg-white p-3 shadow-md shadow-[#e9e9e9] rounded-lg">
                                ...
                                <div className="w-full mb-12 xl:mb-0 px-4 mx-auto mt-4">
                                    <div class="font-semibold mb-3">
                                        <i class="far fa-solid fa-house-circle-xmark mr-3"></i>
                                                    ...
                                                    <div key={index} class="rounded overflow-hidden border">
                                                        ...
                                                        <div class="px-6 py-4">
                                                            <div className="flex flex-wrap items-center">
                                                                ...
                                                                <div className="relative flex-grow flex-1 text-right">
                                                                    <i className="fa-solid fa-trash-can text-[13px] text-[#FF202B] cursor-pointer" onClick={() => handleDelete(row.Pet_lost_id, row.Pet_id)}></i>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="text-[10px] mt-3 my-1">Date Pet Lost : {row.Date_pet_lost}</div>
                                                        </div>
                                                        <div class="px-6 pb-2">
                                                            {row.Vaccine !== "ไม่มี" && (
                                                            ...
                                                            {row.Post_status === 1 ? (
                                                                <div className="flex items-center justify-center mt-3 my-2">
                                                                    <button class="bg-[#28a745] hover:bg-[#218838] text-white text-[12px] font-bold py-2 px-3 rounded" onClick={() => handleEditStatus(row.Pet_lost_id)}>
                                                                        Encountered
                                                                    </button>
                                                                </div>
                                                            ) : (
                                                                <div className="flex items-center justify-center mt-3 my-2">
                                                                    <div className="text-[#28a745] font-bold text-[18px]">เจอแล้ว</div>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
...
export default UserPetLostPost