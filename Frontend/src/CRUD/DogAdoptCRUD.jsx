...
function DogAdoptCRUD({ setActiveTab, setEditData }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // กรองจากสายพันธุ๋
                const filteredBySearch = dogData.filter(item =>
    ...
    const handleDelete = (petAdoptId, PetId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const updatedDogData = dogData.filter((item) => item.Pet_adopt_id !== petAdoptId);
    ...
    return (
        <main className="bg-white-300 flex-1 p-3 overflow-hidden bg-[#f0f2f5]">
            <div className="flex flex-col">
                <div className="flex flex-1 py-4 text-[20px] font-bold ml-4">Dog Adopt</div>
