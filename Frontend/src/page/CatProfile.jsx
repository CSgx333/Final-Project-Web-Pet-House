...
function CatProfile() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const setProfileID = result.data.data.find(item => item.Pet_adopt_id === parseInt(profileData));
                const allData = result.data.data;
                const filteredData = allData
                .filter(item => item.Pet_type_id === 2 && item.Pet_adopt_id !== parseInt(profileData))
                .sort(() => 0.5 - Math.random())
                .slice(0, 4);

                setCatData(setProfileID);
                setAllCatData(filteredData);
            } catch (error) {
                console.error("Error fetching data:", error);
            }
        };
    ...
    const isPetInUserList = () => {
        return petListPost.some(item => item.Pet_adopt_id === catData.Pet_adopt_id && item.Account_id === parseInt(UserId));
    };
    ...
    const handleAddList = () => {
        Swal.fire({
            title: 'Are you sure ?',
            text: 'Do you want to add it to your list ?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                axios.post('http://localhost:5000/user_adopt_list', { ...inputs,
                    AccountId: UserId,
                    PetAdoptId: catData.Pet_adopt_id,
                    ListPetStatus: 2,
                    AdoptDate: null,
                    UserStatus: 2,
                    OwnerStatus: 2
    ...
    const AccountID = catData.Account_id;
    // Fetch Data
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                <div className="flex items-center justify-center my-5">
                    <div class="sm:max-w-[600px] max-w-[400px]">
                        <div className="rounded-md overflow-hidden border bg-[#ffffff]">
                            <img className="sm:w-full sm:h-full w-[25rem] h-[14rem]" src={catData.Image} alt="Pet Image" />
                            <div className="px-6 py-4">
                                <div className="flex flex-wrap items-center">
                                    <div className="font-bold sm:text-[25px] text-[16px] mb-2">Name : {catData.Pet_name}</div>
                                    <div className="relative flex-grow flex-1 text-right cursor-pointer" onClick={openModal}>
                                        <img className="inline-block h-8 w-8 rounded-full mr-3 cursor-pointer" src={catData.UserImage} alt="Account User"/>
                                        <a className="text-gray-700 cursor-pointer">{catData.Name} {catData.Surname}</a>
                                    </div>
                                </div>
                                ...
                                {UserId && catData.Account_id !== parseInt(UserId) && !isPetInUserList() ? (
                                    <div className="flex items-center justify-center mt-3 my-4">
                                        <button onClick={() => handleAddList()} class="text-[12px] relative rounded px-5 py-2.5 overflow-hidden group bg-green-500 relative hover:bg-gradient-to-r hover:from-green-500 hover:to-green-400 text-white hover:ring-2 hover:ring-offset-2 hover:ring-green-400 transition-all ease-out duration-300">
                                            <span class="absolute right-0 w-8 h-32 -mt-12 transition-all duration-1000 transform translate-x-12 bg-white opacity-10 rotate-12 group-hover:-translate-x-40 ease"></span>
                                            <span class="relative"><i class="fa-solid fa-plus pr-2"></i>ADD LIST</span>
                                        </button>
                                    </div>
                                ) : (
                                    null
                    ...
                    <div className="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center">
                        ...
                        <div className="bg-white p-5 rounded shadow-lg max-w-md w-full mx-auto z-50">
                            <div className="flex items-center justify-between text-[20px]">
                                {UserId && catData.Account_id !== parseInt(UserId) && filteredChatRooms.length === 0 ? (
                            ...
                            <div className="flex items-center justify-center mt-5 mb-4">
                                <img
                                    className={`inline-block h-[120px] w-[120px] rounded-full ${
                                        catData.Status === "ใช้งาน" ? "border-4 border-green-500" : ""
                                    }`}
                                    src={catData.UserImage}
                                    alt="Account User"
                                />
                            </div>
                            <a className="flex items-center justify-center text-gray-700 text-[20px] font-bold">{catData.Name} {catData.Surname}</a>
                            <div className="rounded-md overflow-hidden border bg-[#ffffff] mt-3">
                                <div className="px-6 py-4">
                                    ...
                                    <div class="my-2">Email : {catData.Email}</div>
                                    <div class="my-2">Telephone : {catData.Telephone}</div>
                                    <div class="my-2">Province : {catData.UserProvince}</div>
                                </div>
