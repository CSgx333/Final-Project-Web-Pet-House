...
function PetLost() {
    ...
    const linkUserAddPetLost = () => {
        navigate('/AddPetLost');
        window.scrollTo(0, 0);
    }
    const linkLostProfile = (data) => {
    ...
    const [lostEndData, setLostEndData] = useState([]);
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // กรองจาก Breed
                const petBreed = allData.filter(item =>
                    item.Breed.toLowerCase().includes(searchBreed.toLowerCase())
                );

                // กรองจากเพศ
                const petGender = filterGender ? petBreed.filter(item => item.Gender === filterGender) : petBreed;
                
                // กรองจาก Image Breed
                const imgBreed = petGender.filter(item =>
                    item.Breed.toLowerCase().includes(predictedBreed.toLowerCase())
                );

                // กรอง Status
                const petStatus = imgBreed.filter(item => item.Post_status === 1);
                const petEnd = allData.filter(item => item.Post_status === 2);
                const filteredData = petEnd.sort(() => 0.5 - Math.random()).slice(0, 4);
                setLostData(petStatus);
                setLostEndData(filteredData)
            } catch (error) {
    ...
    const currentPageData = lostData.slice(startIndex, endIndex);
    const totalPages = Math.ceil(lostData.length / itemsPerPage);
    const nextPage = () => {
                ...
                <MiniMenu/>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                {!UserId ? (
                                    ...
                                    <div className="relative flex-grow flex-1 text-right">
                                        <button onClick={linkUserAddPetLost} class="box-border z-30 inline-flex items-center justify-center w-auto px-6 py-2 overflow-hidden font-bold text-white transition-all duration-300 bg-indigo-600 rounded-md cursor-pointer group ring-offset-2 ring-1 ring-indigo-300 ring-offset-indigo-200 hover:ring-offset-indigo-500 ease focus:outline-none">
                        ...
                        {lostData.length === 0 ? (
                            ...
                            currentPageData.map((row, index) => (
                                    <div key={index} className="p-2 cursor-pointer group" onClick={() => linkLostProfile(row.Pet_lost_id)}>
                            ...
                            <div className="max-w-7xl mx-auto px-4 py-10 mt-8 sm:px-6 lg:px-8">
                                <div className="p-4 bg-white rounded-lg sm:p-8 bg-gradient-to-r from-[#39414c] to-[#161e28]">
                                    <div className="flex items-center justify-between mb-4">
                                        <h5 className="text-[14px] sm:text-[16px] font-bold leading-none text-gray-900 dark:text-white">At pets that have been found</h5>
                                    </div>
                                    <div className="flow-root">
                                            <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
                                                {lostEndData.map((row, index) => (
                                                    <li key={index} className="py-3 sm:py-4">
                                                        <div className="flex items-center">
                                                            <div className="flex-shrink-0 sm:pr-2">
                                                                <img className="w-10 h-10 rounded-full" src={row.Image} alt="Pet Image"/>
                                                            </div>
