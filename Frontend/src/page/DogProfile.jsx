...
function DogProfile() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                .filter(item => item.Pet_type_id === 1 && item.Pet_adopt_id !== parseInt(profileData))
                .sort(() => 0.5 - Math.random())
                .slice(0, 4);

                setDogData(setProfileID);
                setAllDogData(filteredData);
            } catch (error) {
                console.error("Error fetching data:", error);
            }
        };
    ...
    const isPetInUserList = () => {
        return petListPost.some(item => item.Pet_adopt_id === dogData.Pet_adopt_id && item.Account_id === parseInt(UserId));
    };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.post('http://localhost:5000/user_adopt_list', { ...inputs,
                    AccountId: UserId,
                    PetAdoptId: dogData.Pet_adopt_id,
                    ListPetStatus: 2,
                    AdoptDate: null,
                    UserStatus: 2,
                    OwnerStatus: 2
    ...
    const AccountID = dogData.Account_id;
    // Fetch Data
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                <div className="flex items-center justify-center my-5">
                    <div class="sm:max-w-[600px] max-w-[400px]">
                        <div className="rounded-md overflow-hidden border bg-[#ffffff]">
                            <img className="sm:w-full sm:h-full w-[25rem] h-[14rem]" src={dogData.Image} alt="Pet Image" />
                            <div className="px-6 py-4">
                                <div className="flex flex-wrap items-center">
                                    <div className="font-bold sm:text-[25px] text-[16px] mb-2">Name : {dogData.Pet_name}</div>
                                    <div className="relative flex-grow flex-1 text-right cursor-pointer" onClick={openModal}>
                                        <img className="inline-block h-8 w-8 rounded-full mr-3" src={dogData.UserImage} alt="Account User"/>
                                        <a className="text-gray-700">{dogData.Name} {dogData.Surname}</a>
                                    </div>
                                </div>
                                ...
                                {UserId && dogData.Account_id !== parseInt(UserId) && !isPetInUserList() ? (
                    ...
                    <div className="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center">
                        ...
                        <div className="bg-white p-5 rounded shadow-lg max-w-md w-full mx-auto z-50">
                            <div className="flex items-center justify-between text-[20px]">
                                {UserId && dogData.Account_id !== parseInt(UserId) && filteredChatRooms.length === 0 ? (
                            ...
                            <div className="flex items-center justify-center mt-5 mb-4">
                                <img
                                    className={`inline-block h-[120px] w-[120px] rounded-full ${
                                        dogData.Status === "ใช้งาน" ? "border-4 border-green-500" : ""
                                    }`}
                                    src={dogData.UserImage}
                                    alt="Account User"
                                />
                            </div>
                            <a className="flex items-center justify-center text-gray-700 text-[20px] font-bold">{dogData.Name} {dogData.Surname}</a>
                            <div className="rounded-md overflow-hidden border bg-[#ffffff] mt-3">
                                <div className="px-6 py-4">
                                    ...
                                    <div class="my-2">Email : {dogData.Email}</div>
                                    <div class="my-2">Telephone : {dogData.Telephone}</div>
                                    <div class="my-2">Province : {dogData.UserProvince}</div>
                                </div>
