...
function CatAdopt() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // กรองจาก Breed
                const catBreed = catData.filter(item =>
                    item.Breed.toLowerCase().includes(searchBreed.toLowerCase())
                );

                // กรองจากเพศ
                const catGender = filterGender ? catBreed.filter(item => item.Gender === filterGender) : catBreed;
                
                // กรองจาก Image Breed
                const imgBreed = catGender.filter(item =>
                    item.Breed.toLowerCase().includes(predictedBreed.toLowerCase())
                );

                // กรอง Status
                const catStatus = imgBreed.filter(item => item.Post_status === 1);
    ...
    const [catEndPost, setCatEndPost] = useState([]);
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const catData = filteredData.filter(item => item.Pet_type_id === 2);
                const postStatus = catData.filter(item => item.List_pet_status === 1);
                setCatEndPost(postStatus);
            } catch (error) {
                console.error("Error fetching data:", error);
            }
        };
    ...
    return (
        <div className="font-Sarabun">
            ...
            <main className="py-[50px] bg-[#f1f5f9]">
                <div>
                    <div style={{ backgroundImage: `url(/img/hero5.jpg)` }} className="w-full h-[300px] bg-center bg-cover">
                ...
                <MiniMenu/>
                        ...
                        {catData.length === 0 ? (
                            ...
                            currentPageData.map((row, index) => (
                                    <div key={index} className="p-2 cursor-pointer group" onClick={() => linkCatProfile(row.Pet_adopt_id)}>
