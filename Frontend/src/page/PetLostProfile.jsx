...
function PetLostProfile() {
    ...
    const [allLostData, setAllLostData] = useState([]);
    useEffect(() => {
        const fetchData = async () => {
            try {
                const result = await axios.get('http://localhost:5000/pet_lost');
                const setProfileID = result.data.data.find(item => item.Pet_lost_id === parseInt(profileData));
                ...
                .filter(item => item.Pet_lost_id !== parseInt(profileData))
                .sort(() => 0.5 - Math.random())
                .slice(0, 4);

                setLostData(setProfileID);
                setAllLostData(filteredData);
            } catch (error) {
    ...
    const AccountID = lostData.Account_id;
    // Fetch Data
    useEffect(() => {
        const fetchData = async () => {
            try {
    ...
    return (
        <div className="font-Sarabun">
            ...
            <main className="py-[50px] bg-[#f1f5f9]">
                ...
                <div>
                    <div style={{ backgroundImage: `url(/img/hero10.jpg)` }} className="w-full h-[300px] bg-center bg-cover">
                ...
                <div className="flex items-center justify-center my-5">
                    <div class="sm:max-w-[600px] max-w-[400px]">
                        <div className="rounded-md overflow-hidden border bg-[#ffffff]">
                            <img className="sm:w-full sm:h-full w-[25rem] h-[14rem]" src={lostData.Image} alt="Pet Image" />
                            <div className="px-6 py-4">
                                <div className="flex flex-wrap items-center">
                                    <div className="font-bold sm:text-[25px] text-[16px] mb-2">Name : {lostData.Pet_name}</div>
                                    <div className="relative flex-grow flex-1 text-right cursor-pointer" onClick={openModal}>
                                        <img className="inline-block h-8 w-8 rounded-full mr-3 cursor-pointer" src={lostData.UserImage} alt="Account User"/>
                                        <a className="text-gray-700 cursor-pointer">{lostData.Name} {lostData.Surname}</a>
                                    </div>
                                </div>
                                <p className="text-gray-700 sm:text-base text-[12px] py-1">
                                    Breed : {lostData.Breed}
                                </p>
                                <p className="text-gray-700 sm:text-base text-[12px] py-1">
                                ...
                                <p className="text-gray-700 sm:text-base text-[12px] py-1">
                                    Province : {lostData.Province}
                                </p>
                                <p className="text-gray-700 sm:text-base text-[12px] py-1">
                                    Date Lost : {lostData.Date_pet_lost}
                                </p>
                    ...
                    <div className="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center">
                        ...
                        <div className="bg-white p-5 rounded shadow-lg max-w-md w-full mx-auto z-50">
                            <div className="flex items-center justify-between text-[20px]">
                                {UserId && lostData.Account_id !== parseInt(UserId) && filteredChatRooms.length === 0 ? (
                            ...
                            <div className="flex items-center justify-center mt-5 mb-4">
                                <img
                                    className={`inline-block h-[120px] w-[120px] rounded-full ${
                                        lostData.Status === "ใช้งาน" ? "border-4 border-green-500" : ""
                                    }`}
                                    src={lostData.UserImage}
                                    alt="Account User"
                                />
                            </div>
                            <a className="flex items-center justify-center text-gray-700 text-[20px] font-bold">{lostData.Name} {lostData.Surname}</a>
                            <div className="rounded-md overflow-hidden border bg-[#ffffff] mt-3">
                                <div className="px-6 py-4">
                                    ...
                                    <div class="my-2">Email : {lostData.Email}</div>
                                    <div class="my-2">Telephone : {lostData.Telephone}</div>
                                    <div class="my-2">Province : {lostData.UserProvince}</div>
                                </div>
